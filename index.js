...
function toFileUrl(p) {
  // Windows paths are not expected in Cloud Functions, but keep it safe.
...
function buildWatermarkSvg(W, H, text, fontPath) {
  ...
  const fontUrl = toFileUrl(fontPath);
...
async function renderWatermarkPng(W, H, outPng, fontPath, text = WM_TEXT) {
  ...
  const svg = buildWatermarkSvg(W, H, text, fontPath);
...
export const processMediaWatermark = onObjectFinalized(
  ...
  async (event) => {
    ...
    try {
      ...
      logger.info("watermark config", { fontPath, WM_OPACITY, WM_ANGLE_DEG, WM_TEXT });
      ...
      if (contentType.startsWith("image/")) {
        ...
        await renderWatermarkPng(W, H, tmpWm, fontPath, WM_TEXT);
        ...
        const doc = parseDocId(filePath);
        if (doc) {
          await admin.firestore().doc(`${doc.top}/${doc.docId}`).set(
            { thumbByPath: { [filePath]: thumbUrl } },
            { merge: true }
          );
        }
