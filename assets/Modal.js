...
function startProgressBar(btn, opts = {}) {
  ...
  // 狀態：upload（顯示百分比）/ processing（顯示「處理中...」點點）
  let __mode = "upload";
  let __stopDots = null;
  let __processingBase = "處理中";

  function __clearDots() {
    try { if (typeof __stopDots === "function") __stopDots(); } catch (_) { }
    __stopDots = null;
  }
  ...
  function startProcessing(baseText = "處理中") {
    __mode = "processing";
    __processingBase = baseText || "處理中";
    // 讓進度條停在 100%
    try { update(100); } catch (_) { }
    __clearDots();
    __stopDots = startDots(label, __processingBase);
  }
  ...
  return { update, stop, startProcessing };
}
...
newUrls.push(await getDownloadURL(r));
    ...
    // ⑤ UI 收尾：上傳到 100% 後，改成同位置「處理中...」點點（不要另外開彈窗）
    prog.update(100);
    prog.startProcessing("處理中");
    btn.disabled = true;
...
urls.push(await getDownloadURL(r));
    ...
    // 上傳到 100% 後，改成同位置「處理中...」點點（不要另外開彈窗）
    prog.update(100);
    prog.startProcessing("處理中");
    btn.disabled = true;
    btn.setAttribute("aria-busy", "true");
